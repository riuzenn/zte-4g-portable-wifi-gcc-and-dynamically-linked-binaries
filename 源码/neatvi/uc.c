...
static const struct achar *find_achar(int c)
{
	int h, m, l;
	h = LEN(achars);
	l = 0;
...
{
	const struct achar *a1 = find_achar(c1);
	const struct achar *a2 = find_achar(c2);
...
static int uc_cshape(int cur, int prev, int next)
{
	...
	const struct achar *ac = find_achar(c);
...
static int find(int c, const int tab[][2], int n)
{
	int l = 0;
	int h = n - 1;
	int m;
