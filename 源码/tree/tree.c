...
bool qflag, Nflag, Qflag, Dflag, inodeflag, devflag, hflag;
bool Hflag, siflag, cflag, duflag, pruneflag;
bool noindent, force_color, nocolor, xdev, noreport, flimit, dirsfirst;
...
/* color.c */
extern bool colorize, ansilines;
extern char *endcode;
...
int main(int argc, char **argv)
{
  ...
  Dflag = qflag = Nflag = Qflag = hflag = Hflag = siflag = cflag = FALSE;
  noindent = force_color = nocolor = xdev = noreport = reverse = FALSE;
  ignorecase = matchdirs = dirsfirst = inodeflag = devflag = FALSE;
...
void usage(int n)
{
  ...
  fprintf(n < 2? stderr: stdout,
	"usage: tree [-acdfghilnpqrstuvxACDFQNSU]\n"
	"\t[-L level] [-P pattern] [-I pattern] [-o filename] [--version]\n"
	"\t[--help] [--inodes] [--device] [--noreport] [--dirsfirst]\n"
  ...
  fprintf(stdout,
	...
	"  --charset X   Use charset X for terminal and indentation line output.\n"
...
void indent(int maxlevel)
{
  ...
  if (ansilines) {
    ...
    for(i=0; (i <= maxlevel) && dirs[i]; i++) {
      fprintf(outfile,"%s ",
	      dirs[i+1] ? (dirs[i]==1 ? linedraw->vert : "   ")
