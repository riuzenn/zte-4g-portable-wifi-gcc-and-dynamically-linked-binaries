...
#include "sfm.h"
static void chmod_entry(const Arg *arg);
#include "config.h"
...
get_term_size(void)
{
    ...
    term.rows = ws.ws_row ? ws.ws_row : 30;
    term.cols = ws.ws_col ? ws.ws_col : 100;
}
...
static void
chmod_entry(const Arg *arg)
{
    char mode[32];
    Entry *entry;
    const char *argv[4];  // chmod + mode + path + NULL
    Command cmd;

    if (current_pane->entry_count < 1) {
        print_status(color_warn, "No entry selected.");
        return;
    }

    // 获取用户输入的权限模式
    if (get_user_input(mode, sizeof(mode), "chmod mode: ") != 0)
        return;

    entry = &current_pane->entries[current_pane->current_index];

    // 构造命令参数：chmod 模式 文件路径
    argv[0] = mode;
    argv[1] = entry->fullpath;
    argv[2] = NULL;   // 结束标记

    cmd.cmdv = (char **)chmod_cmd;   // 这里是 "chmod"
    cmd.cmdc = chmod_cmd_len;        // 长度为 1
    cmd.argv = (char **)argv;
    cmd.argc = 2;                    // 只传 mode 和 path，共 2 个参数
    cmd.wait_exec = DontWait;        // 不等待完成
