...
#if defined(HAVE_EX_DATA) && !defined(CURL_WSSL_NO_SESSION)
...
static bool wssl_apply_session(
  ...
  struct Curl_ssl_session *scs)
{
#ifdef CURL_WSSL_NO_SESSION
  (void)cf; (void)data; (void)wss; (void)alpns;
  (void)sess_reuse_cb; (void)scs;
  return FALSE;
#else
